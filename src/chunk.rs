...
impl TryFrom<&Vec<u8>> for Chunk {
  type Error = Error;
  fn try_from(bytes: &Vec<u8>) -> Result<Chunk> {
    let b: [u8; 4] = bytes[4..8].try_into().unwrap();
    let chunk_type = ChunkType::try_from(b).unwrap();
    let idx = bytes.len() - 4;
    ...
    if calc_crc != crc {
      return Err(Box::new(ChunkError::InvalidCrc(calc_crc, crc)));
    }
